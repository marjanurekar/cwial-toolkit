...
67% of respondents had heard the claim, and 42% of THOSE AWARE believed it
true. The target population P is therefore the claim-aware subpopulation,
not all UK adults, and getting that wrong is the kind of error the framework
exists to catch.

The claim-aware base is DERIVED from published percentages (2200 x 0.67 =
1474); no such base is published directly. The derivation is declared here
and matches the published VFRB record.
...
N_AWARE = 1474       # derived: 2200 x 0.67, not a published base
P_BELIEVE = 0.42
...
def main():
    ...
    # Sources for the TRUTH VALUE only. The survey is the measuring
    # instrument, not a reference for V(F), and is registered below instead
    ...
    vfrb.add_source(PROP_ID,
                    "HM Treasury, Public Expenditure Statistical Analyses 2016, "
                    "p.194 Table C1",
                    isrg_grade="I",
                    url="https://assets.publishing.service.gov.uk/media/"
                        "5a805e21ed915d74e33fa05a/PESA_2016_Publication.pdf")
    ...
    reg.register(
        ...
        itype="survey",
        calibration_date="2018-09-28",   # fieldwork date, required for a survey
    ...
    #
    # State the divisor you used, not only the result: "half-width 0.10 at
    ...
    assert abs(result.mdd - 0.25) < 0.01, "MDD (capability) mismatch"
    ...
    print("     threshold (%.3f), i.e. outside the range where the instrument's"
          % result.response_threshold)
    print("     capability would be in question; this is a statement of scale,")
    print("     not proof that the true displacement exceeds that threshold.")
