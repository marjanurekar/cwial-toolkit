...
class UncertaintyBudget:
    ...
    """
    def __init__(self, u_a, label="BDI", verbose=False):
        self.u_a = u_a
        self.label = label
    ...
    def add_type_b(self, name, value, description=""):
        self.type_b[name] = (value, description)
        if self.verbose:
            print(f"[Uncertainty] Added Type B component '{name}' "
    ...
    def critical_value(self):
        """
        Currie critical value, 1.645 * u_c at alpha = 0.05 (Currie 1968;
        ISO 11843-1): the level a reading must exceed for the procedure to
        separate it from the null at 5 per cent false-positive risk.

        This is NOT the laboratory's reported verdict. This package reports
        one verdict per measurement, distinguishable_from_zero, evaluated at
        the stricter criterion |BDI| > U = 2 * u_c (see measurands.py). The
        critical value is quoted alongside it as a capability of the
        procedure, for comparability with the detection literature. See the
    ...
    def summary(self):
        ...
        lines.append(f"     Critical value          : {crit:.4f}   (1.645*u_c, Currie; procedure separates from null)")
